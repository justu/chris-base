...
import com.chris.base.sys.form.PasswordForm;
...
public class SysUserController extends AbstractController {
	...
	public CommonResponse info(){
		return CommonResponse.ok().put("user", getUser());
	}
	...
	public CommonResponse password(@RequestBody PasswordForm form){
		if (ValidateUtils.isBlank(form.getNewPassword())) {
			throw new CommonException("新密码不为能空");
		}
		...
		if(count == 0){
			return CommonResponse.error("原密码不正确");
		}
	...
	public CommonResponse save(@RequestBody SysUserEntity user){
//		ValidatorUtils.validateEntity(user, AddGroup.class);
	...
	public CommonResponse update(@RequestBody SysUserEntity user){
//		ValidatorUtils.validateEntity(user, UpdateGroup.class);
	...
	public CommonResponse delete(@RequestBody Long[] userIds){
		if(ArrayUtils.contains(userIds, 1L)){
			return CommonResponse.error("系统管理员不能删除");
		}
